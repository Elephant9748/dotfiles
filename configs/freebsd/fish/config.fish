...
function fish_greeting
	...
	echo -en (who -bTu | awk '{print " \\\\e[1mUptime: \\\\e[0;32m "$2" "$3" "$4" "$5" "$6" "$7"\\\\e[0m"}')                                         # packages procps
    echo -e (service ntpd status | awk '{if ($3=="running") print " \\\\e[1mntpd: \\\\e[0;32mOn\\\\e[0m";else print " \\\\e[1mNtp: \\\\033[0;31mOff\\\\e[0m"}')
	echo -e (uname -n | awk '{print " \\\\e[1mHostname: \\\\e[0;32m"$0"\\\\e[0m"}')                                                         # packages lsb-release
    echo -e (freebsd-version | awk '{print " \\\\e[1mDescription: \\\\e[0;32m"$0"\\\\e[0m"; exit}')
    echo -e (sysctl hw.model | sed 's/^hw.model: //' | awk '{print " \\\\e[1mCPU: \\\\e[0;32m"$0"\\\\e[0m"; exit}')
    echo -e (pciconf -lv | grep -B3 VGA | awk '{print ""$3" "$4" "$5""}' | sed 's/\'//' | sed 's/\'//' | sed 's/  //' | awk '{print ""$0""}' ORS=' ' | awk '{print " \\\\e[1mVGA: \\\\e[0;32m"$0"\\\\e[0m"; exit}')  # packages pciutils
    ...
    echo -e (echo $XDG_SESSION_TYPE | awk '{if ($0=="") print " \\\\e[1mSession: \\\\e[0;32mtty\\\\e[0m";else print " \\\\e[1mSession: \\\\e[0;32m"$0"\\\\e[0m"}')(echo $XDG_CURRENT_DESKTOP | awk '{print " \\\\e[0;32m"$0"\\\\e[0m"}')
	...
	# )
	echo -en (\
        zpool list -o name,size,free,capacity | grep -i rpool | \
        awk '{printf "\\\\t%s\\\\t\\\\t\\\\t%4s/%4s/%4s \\\\n",$1,$3,$2,$4}' | \
	 	sed -e 's/^\(.*\([8][5-9]\|[9][0-9]\)%.*\)$/\\\\e[0;31m\1\\\\e[0m/' -e 's/^\(.*\([7][5-9]\|[8][0-4]\)%.*\)$/\\\\e[0;33m\1\\\\e[0m/' 

	)
    echo -ne (\
            df -hT | grep -E 'gpt|vtbd|ada' | \
            awk '{printf "\\\\t%s\\\\t%4s/%4s/%4s \\\\n\n",$1,$3,$5,$6}' | \
	 	    sed -e 's/^\(.*\([8][5-9]\|[9][0-9]\)%.*\)$/\\\\e[0;31m\1\\\\e[0m/' -e 's/^\(.*\([7][5-9]\|[8][0-4]\)%.*\)$/\\\\e[0;33m\1\\\\e[0m/' 
	...
	echo
    echo -en (\
            ifconfig -a | grep -E "inet|vtnet|wlan" | \
            awk '{print ""$1" "$2""}' | \
            sed 's/: .*//' | \
            awk 'BEGIN {i=""} /inet/{print i " "$0"";next}//{i=$1}' | \
            awk '{printf "\\\\t%s %4s\\\\n", $1,$3}'
    )
	...
	# 	)
      ...
      echo -e ' note!: '
      set_color normal
      echo -e '\t* upgrade freebsd'
      set_color 6A6362
      echo -e '\t\t- sudo freebsd-update fetch'
      echo -e '\t\t- sudo freebsd-update install'
      set_color normal
      echo -e '\t* tried ports jails'
      set_color normal
      echo 
